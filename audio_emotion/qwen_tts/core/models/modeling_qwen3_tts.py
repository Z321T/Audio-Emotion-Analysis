...
def _fallback_default_rope_init(config, device=None):
    """为缺失 `default` rope 实现的 transformers 版本提供回退初始化。"""
...
class Qwen3TTSTalkerRotaryEmbedding(nn.Module):
    def __init__(self, config: Qwen3TTSTalkerConfig, device=None):
        ...
        if self.rope_type in ROPE_INIT_FUNCTIONS:
            ...
            self.rope_init_fn = _fallback_default_rope_init
...
class Qwen3TTSTalkerModel(Qwen3TTSTalkerTextPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.padding_idx = getattr(config, "pad_token_id", getattr(config, "codec_pad_id", 0))
        self.vocab_size = config.vocab_size
        self.layers = nn.ModuleList(
